...
# --- API НАСТРОЙКИ (ЗАМЕНИТЕ НА ВАШИ РЕАЛЬНЫЕ КЛЮЧИ) ---
# Ключи получать тут: https://yandex.ru
MAPS_API_KEY = "ВАШ_КЛЮЧ_КАРТ"
RASP_API_KEY = "ВАШ_КЛЮЧ_РАСПИСАНИЙ"
...
# --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ЛОГИКА ЯНДЕКСА) ---

def get_city_info(city_name):
    """ Находит координаты и код города для всего мира """
    try:
        # 1. Геокодер: название -> координаты
        ...
        # 2. Ближайший пункт: координаты -> код Яндекса (c213 и т.д.)
...
def get_weather(lat, lon):
    """ Получает погоду в точке прибытия """
...
def get_routes(code_from, code_to):
    """ Получает рейсы (поезда, самолеты) между кодами городов """
    # Ищем рейсы на сегодняшнюю дату
